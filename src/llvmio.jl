...
## ---
    ...
    function printf(n::AbstractFloat)
        fmt = ManualMemory.MemoryBuffer{4, UInt8}(undef)
        fmt.data = (0x25, 0x67, 0x0a, 0x00) # "%g\n"
        GC.@preserve fmt printf(fmt, n)
    end
    function printf(n::Integer)
        fmt = ManualMemory.MemoryBuffer{4, UInt8}(undef)
        fmt.data = (0x25, 0x64, 0x0a, 0x00) # "%d\n"
        GC.@preserve fmt printf(fmt, n)
    end
    function printf(n::Unsigned)
        fmt = ManualMemory.MemoryBuffer{6, UInt8}(undef)
        fmt.data = (0x30, 0x78, 0x25, 0x78, 0x0a, 0x00) # "0x%x\n"
        GC.@preserve fmt printf(fmt, n)
    end

    # Print a vector of numbers
    function printf(v::AbstractVector{<:AbstractFloat})
        fmt = ManualMemory.MemoryBuffer{4, UInt8}(undef)
        fmt.data = (0x25, 0x65, 0x09, 0x00) # "%e\t"
        @inbounds GC.@preserve fmt for i ∈ eachindex(v)
            printf(fmt, v[i])
        end
    end
    function printf(v::AbstractVector{<:Integer})
        fmt = ManualMemory.MemoryBuffer{4, UInt8}(undef)
        fmt.data = (0x25, 0x64, 0x0a, 0x00) # "%d\n"
        @inbounds GC.@preserve fmt for i ∈ eachindex(v)
            printf(fmt, v[i])
    ...
    function printf(v::AbstractVector{<:Unsigned})
        fmt = ManualMemory.MemoryBuffer{6, UInt8}(undef)
        fmt.data = (0x30, 0x78, 0x25, 0x78, 0x0a, 0x00) # "0x%x\n"
        @inbounds GC.@preserve fmt for i ∈ eachindex(v)
            printf(fmt, v[i])
    ...
    function printf(m::AbstractMatrix{<:AbstractFloat})
        fmt = ManualMemory.MemoryBuffer{4, UInt8}(undef)
        fmt.data = (0x25, 0x65, 0x09, 0x00) # "%e\t"
    ...
    function printf(m::AbstractMatrix{<:Integer})
        fmt = ManualMemory.MemoryBuffer{4, UInt8}(undef)
        fmt.data = (0x25, 0x64, 0x09, 0x00) # "%d\t"
        @inbounds GC.@preserve fmt for i ∈ axes(m,1)
            for j ∈ axes(m,2)
                printf(fmt, m[i,j])
            end
    ...
    function printf(m::AbstractMatrix{<:Unsigned})
        fmt = ManualMemory.MemoryBuffer{6, UInt8}(undef)
        fmt.data = (0x30, 0x78, 0x25, 0x78, 0x0a, 0x00) # "0x%x\n"
        @inbounds GC.@preserve fmt for i ∈ axes(m,1)
            for j ∈ axes(m,2)
